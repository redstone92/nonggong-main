...
// 임원진, 지역별 대표 이사 리스트 검색조건
function get_board_sfl_select_options_member($sfl){

    global $is_admin;

    $str = '';
    $str .= '<option value="wr_subject" '.get_selected($sfl, 'wr_subject', true).'>이름</option>';
    $str .= '<option value="wr_1" '.get_selected($sfl, 'wr_1').'>지역</option>';
    $str .= '<option value="wr_2" '.get_selected($sfl, 'wr_2').'>직책</option>';
    $str .= '<option value="wr_4" '.get_selected($sfl, 'wr_4').'>휴대폰</option>';
    $str .= '<option value="wr_5" '.get_selected($sfl, 'wr_5').'>전화</option>';
    $str .= '<option value="wr_6" '.get_selected($sfl, 'wr_6').'>FAX</option>';
    $str .= '<option value="wr_7" '.get_selected($sfl, 'wr_7').'>소속</option>';

    return run_replace('get_board_sfl_select_options', $str, $sfl);
}

// 지역별 현황 리스트 검색조건
function get_board_sfl_select_options_area($sfl){
...
// 단지별 현황 리스트 검색조건
function get_board_sfl_select_options_complex($sfl){

    global $is_admin;

    $str = '';
    $str .= '<option value="wr_1" '.get_selected($sfl, 'wr_1').'>지역</option>';
    $str .= '<option value="wr_2" '.get_selected($sfl, 'wr_2').'>단지명</option>';
    $str .= '<option value="wr_3" '.get_selected($sfl, 'wr_3').'>지정면적</option>';
    $str .= '<option value="wr_4" '.get_selected($sfl, 'wr_4').'>사업기간(년)</option>';
    $str .= '<option value="wr_5" '.get_selected($sfl, 'wr_5').'>총사업비(억원)</option>';
    $str .= '<option value="wr_6" '.get_selected($sfl, 'wr_6').'>사업시행자</option>';
    $str .= '<option value="wr_7" '.get_selected($sfl, 'wr_7').'>공정율(%)</option>';
    $str .= '<option value="wr_8" '.get_selected($sfl, 'wr_8').'>산업용지평균 분양가(원/m²)    </option>';
